...
def create_run_config(hp):
    ...
    return trainer_lib.create_run_config(
        model_dir=os.path.expanduser(FLAGS.output_dir) if hvd.rank() == 0 else None,  # hvd config
        master=FLAGS.master,
...
####################
# porting code from trainer_lib.py, refer to create_experiment code to trim the input_fn
def train_input_fn(hparams):
    ...
    add_problem_hparams(hparams, get_problem_name())

    problem = hparams.problem_instances[0]
    return problem.make_estimator_input_fn(tf.estimator.ModeKeys.TRAIN, hparams)
